...
#define MOD 0x16A6B036D7F2A79ui64
#define NON_RESIDUE 43
static const ui64 f[6] = {
    0,
    0x21840136C85381ui64,
    0x44197B83892AD0ui64,
    0x1400606322B3B04ui64,
    0x1400606322B3B04ui64,
    1
};
...
#define __emulu(a, b) ((uint64_t)((uint32_t)(a)) * (uint64_t)((uint32_t)(b)))
...
#define BAD 0xFFFFFFFFFFFFFFFFui64
...
static void divisor_mul128(const TDivisor* src, ui64 mult_lo, ui64 mult_hi, TDivisor* dst)
{
    ...
    while (!(mult_lo & 1)) {
        ...
        mult_lo >>= 1;
        if (mult_hi & 1)
            mult_lo |= (1ui64 << 63);  // исправлено здесь
        mult_hi >>= 1;
    }
    *dst = cur;
    for (;;) {
...
static void OnActivationIdChange(HWND hDlg)
{
    ...
    installation_id[0] = 0;
    GetDlgItemTextW(hDlg, 101, installation_id, sizeof(installation_id)/sizeof(installation_id[0]));
    ...
    SetDlgItemTextW(hDlg, 103, message);
}
...
{
    if (!ComInitialized) {
        HRESULT status = CoInitialize(NULL); // БЫЛО: CoInitializeEx(NULL, COINIT_APARTMENTTHREADED);
        if (FAILED(status)) {
            MessageBoxW(hParentForMsgBox, strings[10], strings[8], MB_ICONSTOP);
            return FALSE;
        }
        ComInitialized = TRUE;
    }
    ...
    if (FAILED(status)) {
        MessageBoxW(hParentForMsgBox, strings[11], strings[8], MB_ICONSTOP);
        LicenseAgent->Release();
        LicenseAgent = NULL;
        return FALSE;
    }
    ...
    if (dwWPALeft == 0x7FFFFFFF) {
        MessageBoxW(hParentForMsgBox, strings[12], strings[9], MB_ICONWARNING);
        LicenseAgent->Release();
        LicenseAgent = NULL;
        return FALSE;
    }
...
static void GetIdFromSystem(HWND hDlg)
{
    ...
    // Конвертируем strings[7] (wide) в ANSI
    char ansiInfo[256];
    WideCharToMultiByte(CP_ACP, 0, strings[7], -1, ansiInfo, sizeof(ansiInfo), NULL, NULL);
    SetDlgItemTextA(hDlg, 103, ansiInfo);
    ...
    if (FAILED(status) || !installationId) {
        // Конвертируем ошибочные сообщения
        char ansiTitle[256], ansiErr[256];
        WideCharToMultiByte(CP_ACP, 0, strings[13], -1, ansiErr, sizeof(ansiErr), NULL, NULL);
        WideCharToMultiByte(CP_ACP, 0, strings[8], -1, ansiTitle, sizeof(ansiTitle), NULL, NULL);
        MessageBoxA(hDlg, ansiErr, ansiTitle, MB_ICONSTOP);
    } else {
        char ansiInstallationId[256];
        WideCharToMultiByte(CP_ACP, 0, installationId, -1, ansiInstallationId, sizeof(ansiInstallationId), NULL, NULL);
        SetDlgItemTextA(hDlg, 101, ansiInstallationId);
        SysFreeString(installationId);
    }

    // Если OnActivationIdChange требует тоже ANSI строки — конвертируй там по аналогии
...
static void PutIdToSystem(HWND hDlg)
{
    ...
    ULONG dwRetCode;
    char confirmationId[256];
    GetDlgItemTextA(hDlg, 103, confirmationId, sizeof(confirmationId));

    char ansiString[256];
    WideCharToMultiByte(CP_ACP, 0, strings[7], -1, ansiString, 256, NULL, NULL);
    SetDlgItemTextA(hDlg, 103, ansiString);
    EnableWindow(GetDlgItem(hDlg, 102), FALSE);
    EnableWindow(GetDlgItem(hDlg, 104), FALSE);
    UpdateWindow(hDlg);

    BSTR confirmationIdBstr = SysAllocStringLen(NULL, strlen(confirmationId));
    MultiByteToWideChar(CP_ACP, 0, confirmationId, -1, confirmationIdBstr, strlen(confirmationId) + 1);
    ...
    MessageBoxA(hDlg, ansiString, ansiString, MB_ICONINFORMATION);
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	for (i = 0; i < 14; i++)
		LoadStringW(NULL, i, strings[i], sizeof(strings[i]) / sizeof(strings[i][0]));
    ...
    INT_PTR status = DialogBoxW(hInstance, MAKEINTRESOURCEW(100), NULL, (DLGPROC)DialogProc);
